...
def scan(ip): 
    ...
    my_mac = os.popen("ip addr | grep 'state UP' -A1 | tail -n1 | awk '{print $2}' | cut -f1  -d'/'").read().upper() if system() != 'Windows' else 'Unknow' 
    devices_mac = os.popen("grep MAC "+ output + " | awk '{print $3}'").read() + my_mac
    devices_name = os.popen("grep MAC " + output + " | awk '{print $4 ,S$5 $6}'").read() + "\033[1;32m(This device)\033[1;m"
